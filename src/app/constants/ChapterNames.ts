export class ChapterNames {
    ...
    // Method to get index given a chapter name in any language.
  // It searches all three arrays and returns the index if found, or -1 otherwise.
  static getChapterIndex(chapterName: string): number {
    const lowerName = chapterName.toLowerCase();

    let index = this.chapNamesInEnglish.findIndex(name => name.toLowerCase() === lowerName);
    if (index !== -1) { return index; }

    index = this.chapNamesInTamil.findIndex(name => name === chapterName); // assuming exact match for Tamil
    if (index !== -1) { return index; }

    index = this.chapNamesInTelugu.findIndex(name => name === chapterName); // assuming exact match for Telugu
    return index;
  }

  // Method to get the chapter name given an index and language.
  // The language parameter can be 'english', 'tamil' or 'telugu'
  static getChapterName(index: number, language: string): string {
    switch (language.toLowerCase()) {
      case 'english':
        return this.chapNamesInEnglish[index] || '';
      case 'tamil':
        return this.chapNamesInTamil[index] || '';
      case 'telugu':
        return this.chapNamesInTelugu[index] || '';
      default:
        return '';
    }
  }
